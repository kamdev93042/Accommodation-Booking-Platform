  ...
  function uploadPhoto(ev) {
    ...
    axios
      ...
      .then(res => {
        ...
        const filenames = res.data.map(file => file.filename);
        onChange(prev => [...prev, ...filenames]);
      });
  }
    ...
    <>
      ...
      <div className="flex gap-2">
        ...
        <button
          ...
          className="bg-gray-200 w-initial px-4 rounded-2xl"
        >
          Add&nbsp;photo
        </button>
      </div>
      ...
      <div className="mt-2 grid gap-2 grid-cols-3 md:grid-cols-4 lg:grid-cols-8">
        {addedPhotos.length > 0 &&
          addedPhotos.map(filename => (
            <div key={filename} className="flex relative">
              ...
              <button
                onClick={() => selectAsMainPhoto(filename)}
                type="button"
        ...
        {/* Upload from device */}
        <label className="h-32 cursor-pointer flex items-center justify-center gap-1 border bg-transparent rounded-2xl p-1 text-2xl text-gray-600">
          <input
            className="hidden"
            multiple
            type="file"
